...
class GStreamerDetectionApp(GStreamerApp):
    ...
    def get_pipeline_string(self):
        ...
        pipeline_string = (
            "rtspsrc location=rtsp://admin:admin123@192.168.1.2:554/Streaming/Channels/801/ name=src_0 ! "
            + QUEUE("hailo_preprocess_q_0", max_size_buffers=5)
            + "decodebin ! "
            + "videoconvert ! "
            + "videoscale qos=false ! "
            + QUEUE("queue_hailonet", max_size_buffers=30)
            + f"hailonet hef-path={self.hef_path} batch-size={self.batch_size} {self.thresholds_str} force-writable=true ! "
            + QUEUE("queue_hailofilter", max_size_buffers=30)
