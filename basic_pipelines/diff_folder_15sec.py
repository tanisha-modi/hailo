...
def app_callback(pad, info, user_data):
    ...
    person_class = 1
    helmet_violation_class = 4  # Classes to check for violation
    suit_violation_class = 5 

    person_results = [detection for detection in detections if detection.get_class_id() == person_class]
    helmet_violation_results = [detection for detection in detections if detection.get_class_id() == helmet_violation_class]
    suit_violation_results = [detection for detection in detections if detection.get_class_id() == suit_violation_class]
    ...
    helmet_current_violation_ids = [int(result.get_objects_typed(hailo.HAILO_UNIQUE_ID)[0].get_id()) for result in helmet_violation_results if (result.get_objects_typed(hailo.HAILO_UNIQUE_ID)[0].get_id()) is not None]
    suit_current_violation_ids = [int(result.get_objects_typed(hailo.HAILO_UNIQUE_ID)[0].get_id()) for result in suit_violation_results if (result.get_objects_typed(hailo.HAILO_UNIQUE_ID)[0].get_id()) is not None]


    current_time = time.time()

    # Initialize violation_tracker if it doesn't exist
    if not hasattr(user_data, 'helmet_violation_tracker'):
        user_data.helmet_violation_tracker = {}
        
    if not hasattr(user_data, 'suit_violation_tracker'):
        user_data.suit_violation_tracker = {}

    # Update violation tracker
    for violation_id in helmet_current_violation_ids:
        if violation_id not in user_data.helmet_violation_tracker:
            user_data.helmet_violation_tracker[violation_id] = current_time
            
    for violation_id in suit_current_violation_ids:
        if violation_id not in user_data.suit_violation_tracker:
            user_data.suit_violation_tracker[violation_id] = current_time
            
            
    
    # Remove old violations that are no longer detected
    user_data.helmet_violation_tracker = {vid: timestamp for vid, timestamp in user_data.helmet_violation_tracker.items() if vid in helmet_current_violation_ids}
    user_data.suit_violation_tracker = {vid: timestamp for vid, timestamp in user_data.suit_violation_tracker.items() if vid in suit_current_violation_ids}
    
    
    print("helmet ", user_data.helmet_violation_tracker)
    print("suit ", user_data.suit_violation_tracker, "\n")

    # Check for persistent violations and capture image if needed
    image_dir = "captured"
    helmet_dir = os.path.join(image_dir, "helmet")
    for violation_id, first_detection_time in user_data.helmet_violation_tracker.items():
        if current_time - first_detection_time > 3:  # 15 seconds threshold
            if violation_id not in user_data.tracker_ids:
                if not any(person_id in user_data.tracker_ids for person_id in current_person_ids):
                    img_filename = os.path.join(helmet_dir, f"helmet_violation_{violation_id}_{int(current_time)}.jpg")
                    if frame is not None and not frame.size == 0:
                        ...
                        print(f"Violation {violation_id} persisted for >3 seconds. Image saved to {img_filename}.")
                    else:
                        print("Error: The image frame is empty.")
                        
                        
    image_dir = "captured"
    suit_dir = os.path.join(image_dir, "suit")
    for violation_id, first_detection_time in user_data.suit_violation_tracker.items():
        if current_time - first_detection_time > 3:  # 15 seconds threshold
            if violation_id not in user_data.tracker_ids:
                if not any(person_id in user_data.tracker_ids for person_id in current_person_ids):
                    img_filename = os.path.join(suit_dir, f"suit_violation_{violation_id}_{int(current_time)}.jpg")
                    if frame is not None and not frame.size == 0:
                        frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                        cv2.imwrite(img_filename, frame)
                        user_data.tracker_ids.update(current_person_ids)
...
class GStreamerDetectionApp(GStreamerApp):
    def __init__(self, args, user_data):
        ...
        self.rtsp_sources1 = {
            ...
            0: "rtsp://admin:akshit%409977@192.168.1.3:554/Streaming/channels/101/"
            #1: "rtsp://admin:akshit@9977@192.168.1.3:554/Streaming/Channels/201/",
            #0: "rtsp://192.168.1.24:8555/Stream",
            #1: "rtsp://192.168.1.21:8555/live"
            #2: "rtsp://admin:akshit@9977@192.168.1.3:554/Streaming/Channels/301/",
            #3: "rtsp://admin:akshit@9977@192.168.1.3:554/Streaming/Channels/401/",
            #4: "rtsp://admin:akshit@9977@192.168.1.3:554/Streaming/Channels/501/",
            #5: "rtsp://admin:akshit@9977@192.168.1.3:554/Streaming/Channels/601/",
            #6: "rtsp://admin:akshit@9977@192.168.1.3:554/Streaming/Channels/701/",
            #7: "rtsp://admin:akshit@9977@192.168.1.3:554/Streaming/Channels/801/",
          #  8: "rtsp://admin:admin123@192.168.1.2:554/Streaming/Channels/901/",
          #  9: "rtsp://admin:admin123@192.168.1.2:554/Streaming/Channels/1001/",
          #  10: "rtsp://admin:admin123@192.168.1.2:554/Streaming/Channels/1101/",
          #  11: "rtsp://admin:admin123@192.168.1.2:554/Streaming/Channels/1201/",
    ...
    def create_rtsp_sources(self, num_of_src, decode_scale_elements):
        ...
        for n, src_url in self.rtsp_sources1.items():
            ...
            # print({src_url}, {n})
            rtsp_sources += (
                f"rtspsrc location={src_url} name=source_{n} latency=100 message-forward=true ! "
